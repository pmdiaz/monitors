			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="name" title="${message(code: 'monitor.name.label', default: 'Name')}" />
				...
				<g:each in="${monitorInstanceList}" status="i" var="monitorInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${monitorInstance.id}">${fieldValue(bean: monitorInstance, field: "name")}</g:link></td>
