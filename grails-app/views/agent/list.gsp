			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="agentName" title="${message(code: 'agent.agentName.label', default: 'Agent Name')}" />
					
						<th><g:message code="agent.currentStatus.label" default="Current Status" /></th>
				...
				<g:each in="${agentInstanceList}" status="i" var="agentInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${agentInstance.id}">${fieldValue(bean: agentInstance, field: "agentName")}</g:link></td>
					
						<td>${fieldValue(bean: agentInstance, field: "currentStatus")}</td>
